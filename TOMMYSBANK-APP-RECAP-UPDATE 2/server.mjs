...
function completeSignal(signal, reason, now = new Date()) {
  ...
  signal.tracking.finalX = Number(signal.tracking.currentX) || null;
  signal.tracking.finalPriceUsd = Number(signal.tracking.currentPriceUsd) || null;
  signal.tracking.finalMarketCapUsd = Number(signal.tracking.marketCapUsd) || null;
