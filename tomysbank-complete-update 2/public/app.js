  ...
  if (!ranked.length) {
    leaderboardList.innerHTML = `
      <div class="leaderboard-empty">
        La classifica si popolerà con i prossimi segnali monitorati.<br>
        Qui mostreremo soltanto picchi registrati dal chart.
      </div>`;
    return;
  }
  ...
  leaderboardList.replaceChildren(...ranked.map((signal, index) => {
    ...
    card.innerHTML = `
      ...
      <span class="leader-token">
        ...
        <span>Call ${relativeTime(signal.createdAt)} · ora ${formatX(signal.tracking?.currentX)}</span>
      </span>
    ...
    latest.innerHTML = `
      <div class="empty-state">
        <div>
          ...
          <p>Nessuno storico importato. Il prossimo nuovo segnale di TOMYSBANK apparirà qui in tempo reale.</p>
        </div>
      </div>`;
  ...
  try {
    ...
    if (state.source?.channel) {
      document.querySelector("#sourceLabel").textContent =
        state.source.channel === "mad_apes_gambles" ? "TOMYSBANK" : `@${state.source.channel}`;
    }
    renderAll();
  } catch {
...
window.addEventListener("appinstalled", () => showToast("TOMYSBANK installata ✓"));
setInterval(() => {
