...
function setAuthMode(mode) {
  ...
  document.querySelector("#authSubmit").textContent = t(
    state.authMode === "register" ? "memberRegister" : "memberLogin"
  );
  setAuthMessage("");
}
...
function renderFeatured(signal, animate = false) {
  ...
  setPerformanceText(fragment.querySelector("[data-tracking='currentX']"), tracking.currentX);
...
function historyCard(signal) {
  ...
  const peakX = signal.tracking?.peakX;
  const finalX = signal.tracking?.finalX ?? signal.tracking?.currentX;
  const meta = active
...
authForm.addEventListener("submit", async (event) => {
  ...
  if (state.authMode === "register") {
    const { data, error } = await state.supabase.auth.signUp({
      ...
      password,
      options: {
        emailRedirectTo: state.authConfig?.membershipSiteUrl || window.location.origin
      }
    });
    if (error) {
