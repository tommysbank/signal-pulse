...
function extractAddress(text) {
  const codeAddress = text.match(/`\s*(0x[a-fA-F0-9]{40}|[1-9A-HJ-NP-Za-km-z]{32,44})\s*`/m)?.[1];
  ...
  const labelled = text.match(/(?:CA|C\/A|contract|contract\s*address|address|token\s*address)\s*[:\-]\s*(0x[a-fA-F0-9]{40}|[1-9A-HJ-NP-Za-km-z]{32,44})/i)?.[1];
  if (labelled) return labelled;

  const tokenUrl = text.match(/(?:pump\.fun\/(?:coin\/)?|gmgn\.ai\/[^ \n]*?(?:token|coin)[/=\/]|birdeye\.so\/[^ \n]*?(?:token|coin)[/=\/])(0x[a-fA-F0-9]{40}|[1-9A-HJ-NP-Za-km-z]{32,44})/i)?.[1];
...
function chainHeaderMatch(text) {
  return text.match(/^\s*(?:🚨|🔥|🟢|🦍|💎|⚡)?\s*\(?\s*(SOL|ETH|BASE|RBH)\s*\)?\s+(.{2,80})$/imu);
}
...
function classifyPublicSignal(text, address) {
  ...
  if (score < 3) return { accepted: false, reason: `low-confidence-${score}` };
