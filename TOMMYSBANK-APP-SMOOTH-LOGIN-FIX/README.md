...
presenti, ma recupera eventuali call verificati degli ultimi 20 minuti per non
perdere segnali usciti durante deploy o riavvii. Controlla la pagina pubblica
ogni 3 secondi e i reply con gli aggiornamenti `2x`, `5x`, `now`, ecc. non
generano nuovi segnali.
...
cap e moltiplicatore ogni 2 secondi. Conserva sia il moltiplicatore
...
La frequenza è configurabile con `SCRAPE_INTERVAL_MS`, con un minimo di 2,5
secondi. `PRICE_INTERVAL_MS` ha un minimo di 2 secondi. `STARTUP_CATCHUP_MINUTES`
decide quanti minuti recenti recuperare dopo un riavvio, mentre
`MAX_SIGNAL_AGE_MINUTES` evita di pubblicare come nuovi segnali troppo vecchi.
In caso di errori o rate limit il sistema applica automaticamente un backoff
fino a un minuto.

Aggiornamento qualità segnali: il lettore Telegram analizza anche i link
interni dei post pubblici, quindi intercetta meglio i contratti nascosti dietro
testi cliccabili. I link Dexscreener/pair vengono esclusi dalla scelta automatica
dell'address per evitare che una pair venga scambiata per il contratto del token.
Lo status API salva anche gli ultimi motivi di scarto (`lastRejectReasons` e
`lastRejectedSamples`), utile per capire subito perché un post non è diventato
segnale.

Aggiornamento grafico live: sotto al segnale principale viene mostrato un chart
live esterno Dexscreener appena la pair viene rilevata. Il mini grafico interno
resta utile per lo storico e il picco salvato dall'app, mentre il chart esterno
serve per guardare il mercato live senza aspettare i tick del nostro tracking.
Per i token Solana viene mostrato anche il pulsante Pump.fun.

Aggiornamento stabilità Render: il container usa `node:22-bookworm-slim` invece
di Alpine per ridurre crash runtime/segfault anomali. È stato aggiunto
`NODE_OPTIONS=--max-old-space-size=384`, log diagnostici per errori non gestiti e
shutdown ordinato con salvataggio stato quando Render riavvia o ridistribuisce il
servizio. `/healthz` include anche uptime e memoria di base.
