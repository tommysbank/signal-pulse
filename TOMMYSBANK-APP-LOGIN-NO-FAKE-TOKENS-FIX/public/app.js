...
const ACCESS_OK_KEY = "tomysbank-access-ok-at";
const ACCESS_OK_GRACE_MS = 30 * 24 * 60 * 60 * 1000;
...
const translations = {
  en: {
    ...
    liveExternalChartEyebrow: "",
    liveExternalChartTitle: "",
    liveExternalChartCopy: "",
    chartUnavailable: "",
    openLiveChart: "Open full chart",
    openPumpFun: "Open Pump.fun",
    openDexscreener: "Open chart",
...
document.querySelector("#liveChartSection")?.remove();
document.querySelectorAll(".live-chart-section").forEach((element) => element.remove());

const latest = document.querySelector("#latestSignal");
const liveChartSection = null;
const liveChartCard = null;
const liveChartOpen = null;
const liveChartActions = null;
...
function rememberAccessOk() {
  try {
    localStorage.setItem(ACCESS_OK_KEY, String(Date.now()));
  } catch {
    // The server remains the source of truth for access.
  }
}

function clearAccessOk() {
  try {
...
function hasRecentAccessOk() {
  const value = Number(localStorage.getItem(ACCESS_OK_KEY) || 0);
  return Number.isFinite(value) && value > 0 && Date.now() - value < ACCESS_OK_GRACE_MS;
}
...
function hasConfirmedIncomingMarket(signal) {
  ...
  if (tracking.lifecycle === "completed") return true;
  return Boolean(tracking.marketConfirmedAt || tracking.entryPriceUsd || tracking.pairUrl || tracking.pairAddress);
}

function sanitizeIncomingSignals(signals) {
  return Array.isArray(signals)
    ? signals.map(sanitizeIncomingSignal).filter(hasConfirmedIncomingMarket)
    : [];
}
