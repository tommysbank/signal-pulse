...
const priceInterval = Math.max(3_000, Number(process.env.PRICE_INTERVAL_MS || 4_000));
const startupCatchupWindow = 0;
const maxSignalPublishAge = Math.min(
  10 * 60_000,
  Math.max(2 * 60_000, Number(process.env.MAX_SIGNAL_AGE_MINUTES || 6) * 60_000)
);
const marketConfirmationWindow = Math.max(2 * 60_000, Number(process.env.PAIR_CONFIRMATION_MINUTES || 8) * 60_000);
...
function requiresMarketConfirmation(signal) {
  return signal?.source !== "demo";
}

function isMarketConfirmed(signal) {
  const tracking = signal?.tracking || {};
  return Boolean(
    tracking.marketConfirmedAt
    || tracking.entryPriceUsd
    || tracking.pairUrl
    || tracking.pairAddress
  );
}
...
function isVisibleSignal(signal) {
  ...
  if (signal.tracking.lifecycle === "completed") return true;
  if (!requiresMarketConfirmation(signal)) return true;
  return isMarketConfirmed(signal);
}

function visibleSignals() {
  return signals.filter(isVisibleSignal);
}

function rejectUnconfirmedSignal(signal, reason = "pair-not-found") {
  ...
  trackServerEvent("app_signal_rejected_market_confirmation", {
    level: "warning",
    signalId: signal.id,
    ticker: signal.ticker,
    chain: signal.chain,
  ...
  sourceStatus.lastSignalAt = visibleSignals()[0]?.createdAt || null;
  return true;
}
...
async function restoreRuntimeState() {
  ...
  try {
    ...
    for (const signal of saved.signals || []) {
      ...
      if (requiresMarketConfirmation(signal) && !isVisibleSignal(signal) && age > marketConfirmationWindow) {
        if (signal.address) seenAddresses.delete(signal.address.toLowerCase());
        continue;
      }
      signals.push(signal);
...
function notifySignal(signal) {
  signal.tracking ||= {};
  if (signal.tracking.notifiedAt) return;
  signal.tracking.notifiedAt = new Date().toISOString();
...
function publish(signal) {
  ...
  if (requiresMarketConfirmation(signal) && !isMarketConfirmed(signal)) {
    signal.tracking.status = "pending-market";
    signal.tracking.marketConfirmationStartedAt ||= new Date().toISOString();
    trackServerEvent("app_signal_pending_market_confirmation", {
      signalId: signal.id,
      ticker: signal.ticker,
      chain: signal.chain,
      addressEnding: signal.address.slice(-8),
...
async function updateTrackedPrices(targetSignals = signals) {
  ...
  try {
    ...
    for (const signal of targetSignals) {
      ...
      if (!chainId || !signal.address) {
        const confirmationStartedAt = new Date(signal.tracking.marketConfirmationStartedAt || signal.createdAt).getTime();
        const confirmationAge = Date.now() - confirmationStartedAt;
        if (requiresMarketConfirmation(signal) && !isVisibleSignal(signal) && confirmationAge >= marketConfirmationWindow) {
          if (rejectUnconfirmedSignal(signal, "unsupported-or-missing-chain")) {
            updated = true;
          }
        }
        continue;
      ...
      for (let index = 0; index < group.length; index += 30) {
        ...
        try {
          ...
          for (const signal of chunk) {
            ...
            if (!pair) {
              signal.tracking ||= {};
              signal.tracking.status = isVisibleSignal(signal) ? "waiting-pair" : "pending-market";
              ...
              const confirmationAge = Date.now() - confirmationStartedAt;
              if (requiresMarketConfirmation(signal) && !isVisibleSignal(signal) && confirmationAge >= marketConfirmationWindow) {
                if (rejectUnconfirmedSignal(signal, "pair-not-confirmed")) {
                  updated = true;
                }
                continue;
              }
            ...
            if (applyPair(signal, pair)) {
              updated = true;
              if (!wasVisible && isVisibleSignal(signal)) {
                notifySignal(signal);
              } else {
                broadcastEvent("price", publicSignal(signal));
              }
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "GET" && url.pathname === "/api/signals") {
      ...
      return json(res, 200, {
        signals: visible.map(publicSignal),
    ...
    if (req.method === "GET" && url.pathname === "/api/status") {
      if (!await requireMember(req, res)) return;
      const visible = visibleSignals();
      const activeSignals = visible.filter((signal) => signal.tracking?.lifecycle !== "completed").length;
      return json(res, 200, {
        ok: !sourceStatus.lastError,
        source: publicSourceStatus(),
        signalCount: visible.length,
        activeSignals,
        completedSignals: visible.length - activeSignals,
        pushSubscriptions: subscriptions.size,
