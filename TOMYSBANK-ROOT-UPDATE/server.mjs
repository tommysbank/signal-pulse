...
async function sendPushPayload(payload, targetSubscriptions = [...subscriptions.values()]) {
  if (!targetSubscriptions.length) return { sent: 0, failed: 0 };
  if (!process.env.VAPID_PUBLIC_KEY || !process.env.VAPID_PRIVATE_KEY) {
    return { sent: 0, failed: targetSubscriptions.length };
  }
  try {
    const webpush = await getWebPush();
    if (!webpush) return { sent: 0, failed: targetSubscriptions.length };
    const outcomes = await Promise.all(targetSubscriptions.map(async (subscription) => {
      try {
