...
function setLanguage(language) {
  ...
  languageButton.textContent = state.language === "it" ? "🇮🇹 IT" : "🇬🇧 EN";
  localize();
  document.querySelector("#authSubmit").textContent = t("memberLogin");
  updateAccountUi();
  renderAll();
...
function isKnownInvalidSignal(signal) {
  ...
  const address = normalizeAddress(signal?.address);
  return ticker === "TOKE"
...
function isReasonableX(value) {
  ...
  return Number.isFinite(number) && number > 0 && number <= MAX_REASONABLE_X;
}

function safeX(value) {
  return isReasonableX(value) ? Number(value) : null;
}

function hasConfirmedIncomingMarket(signal) {
  if (!signal || isKnownInvalidSignal(signal)) return false;
  if (signal.source === "demo") return true;
  const tracking = signal.tracking || {};
  return isReasonableX(tracking.currentX || tracking.peakX || 1)
...
function sanitizeSignals(signals) {
  return (Array.isArray(signals) ? signals : [])
    .filter(hasConfirmedIncomingMarket)
    .map((signal) => {
      const copy = typeof structuredClone === "function" ? structuredClone(signal) : JSON.parse(JSON.stringify(signal));
      const tracking = copy.tracking || {};
      for (const key of ["currentX", "peakX", "finalX"]) {
        if (tracking[key] != null && !isReasonableX(tracking[key])) tracking[key] = null;
      }
      if (Array.isArray(tracking.history)) {
        tracking.history = tracking.history.filter((point) => isReasonableX(point?.x));
      }
...
function pruneInvalidSignals() {
  const before = state.signals.length;
  state.signals = sanitizeSignals(state.signals);
  if (state.selectedSignalId && !state.signals.some((signal) => signal.id === state.selectedSignalId)) {
...
function pumpFunUrl(signal) {
  ...
  if (signal?.chain === "SOL" || /pump$/i.test(address)) {
...
function recapSignals() {
  ...
  return state.signals
    ...
    .filter((signal) => safeX(signal.tracking?.peakX));
}
...
function renderRecap() {
  ...
  const totalX = periodSignals.reduce((sum, signal) => sum + (safeX(signal.tracking.peakX) || 0), 0);
...
function renderFeatured(signal, animate = false) {
  ...
  const displayX = completed ? safeX(tracking.peakX) ?? safeX(tracking.finalX) : safeX(tracking.currentX);
  ...
  const pumpLink = fragment.querySelector(".pump-trade");
  const pumpUrl = pumpFunUrl(signal);
  if (pumpLink) {
    pumpLink.href = pumpUrl || "#";
    pumpLink.hidden = !pumpUrl;
  }
...
function activeCard(signal) {
  ...
  const currentX = safeX(signal.tracking?.currentX);
  const peakX = safeX(signal.tracking?.peakX);
  card.innerHTML = `
...
function historyCard(signal) {
  ...
  const peakX = safeX(signal.tracking?.peakX);
  const finalX = safeX(signal.tracking?.peakX) ?? safeX(signal.tracking?.finalX) ?? safeX(signal.tracking?.currentX);
  const meta = active
...
function renderLeaderboard() {
  const ranked = state.signals
    .filter((signal) => safeX(signal.tracking?.peakX))
    .sort((a, b) => (safeX(b.tracking.peakX) || 0) - (safeX(a.tracking.peakX) || 0))
    .slice(0, 5);
...
function receiveSignal(signal) {
  if (!hasConfirmedIncomingMarket(signal)) return;
  if (state.signals.some((item) => item.id === signal.id)) return;
  state.signals.unshift(sanitizeSignals([signal])[0]);
  renderAll();
...
function receivePrice(signal) {
  if (!hasConfirmedIncomingMarket(signal)) {
  ...
  if (index < 0) return;
  state.signals[index] = sanitizeSignals([signal])[0];
  renderAll();
  try {
    localStorage.setItem("cached-signals", JSON.stringify(state.signals.slice(0, 100)));
  } catch {
...
async function initializePrivateApp() {
  ...
  try {
    ...
    const payload = await response.json();
    state.signals = sanitizeSignals(payload.signals || []);
    ...
    state.signals = sanitizeSignals(JSON.parse(localStorage.getItem("cached-signals") || "[]"));
    renderAll();
    showToast(t("offline"));
  }
