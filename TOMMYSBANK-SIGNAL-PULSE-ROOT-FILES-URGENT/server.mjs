...
const recentBackfillWindow = Math.max(2 * 60_000, Number(process.env.RECENT_BACKFILL_MINUTES || 20) * 60_000);
const maxReasonableX = Math.max(50, Number(process.env.MAX_REASONABLE_X || 1000));
...
function isReasonableX(value) {
  const number = Number(value);
  return Number.isFinite(number) && number > 0 && number <= maxReasonableX;
}

function hasMarketConfirmation(signal) {
  const tracking = signal?.tracking || {};
  return !isKnownInvalidSignal(signal)
    && Number.isFinite(Number(tracking.entryPriceUsd))
    && Number(tracking.entryPriceUsd) > 0
    && Boolean(tracking.marketConfirmedAt || tracking.pairUrl || tracking.pairAddress);
}

function isVisibleSignal(signal) {
  if (isKnownInvalidSignal(signal)) return false;
  if (signal?.source === "demo") return true;
  if (
    signal?.tracking?.lifecycle === "completed"
    && isReasonableX(signal?.tracking?.peakX || signal?.tracking?.finalX)
  ) {
    return true;
  }
  return hasMarketConfirmation(signal);
}

function isRecentUnconfirmed(signal) {
  const createdAt = new Date(signal?.createdAt || 0).getTime();
  return Number.isFinite(createdAt) && Date.now() - createdAt < recentBackfillWindow;
}

function publicSignal(signal) {
  const { sourceUrl, ...visibleSignal } = signal;
  if (visibleSignal.tracking) {
    visibleSignal.tracking = { ...visibleSignal.tracking };
    if (!isReasonableX(visibleSignal.tracking.currentX)) visibleSignal.tracking.currentX = null;
    if (!isReasonableX(visibleSignal.tracking.peakX)) visibleSignal.tracking.peakX = null;
    if (!isReasonableX(visibleSignal.tracking.finalX)) visibleSignal.tracking.finalX = null;
    if (Array.isArray(visibleSignal.tracking.history)) {
      visibleSignal.tracking.history = visibleSignal.tracking.history.filter((point) => point?.x == null || isReasonableX(point.x));
    }
  }
...
async function restoreRuntimeState() {
  ...
  try {
    ...
    for (const signal of saved.signals || []) {
      ...
      if (isKnownInvalidSignal(signal)) {
        if (signal.address) seenAddresses.delete(signal.address.toLowerCase());
        continue;
      }
      if (!isVisibleSignal(signal) && !isRecentUnconfirmed(signal)) {
        if (signal.address) seenAddresses.delete(signal.address.toLowerCase());
        continue;
      }
      if (Array.isArray(signal.tracking.history)) {
        signal.tracking.history = signal.tracking.history.filter((point) => isReasonableX(point?.x) || point?.x == null);
      }
      if (!isReasonableX(signal.tracking.currentX)) signal.tracking.currentX = null;
      if (!isReasonableX(signal.tracking.peakX)) signal.tracking.peakX = null;
      if (!isReasonableX(signal.tracking.finalX)) signal.tracking.finalX = null;
      signals.push(signal);
    }
...
function announceSignal(signal) {
  if (!signal || signal.announcedAt) return false;
  signal.announcedAt = new Date().toISOString();
...
async function confirmMarketForSignal(signal) {
  if (signal?.source === "demo") return true;
  if (isKnownInvalidSignal(signal)) return false;
  const chainId = chainIds[signal.chain];
  if (!chainId || !signal.address) return false;
  try {
    const pairs = await fetchDexPairs(chainId, [signal]);
    const pair = choosePair(signal, pairs);
    return Boolean(pair && applyPair(signal, pair));
  } catch (error) {
    console.warn(`Conferma pair ${signal.ticker || signal.address}:`, error.message);
    return false;
  }
}

async function publish(signal) {
  if (!signal || isKnownInvalidSignal(signal)) return false;
  const addressKey = signal.address.toLowerCase();
  if (signals.some((item) => item.id === signal.id || item.address?.toLowerCase() === addressKey)) {
    return false;
  }

  if (signal.source !== "demo") {
    const confirmed = await confirmMarketForSignal(signal);
    if (!confirmed || !isVisibleSignal(signal)) return false;
  }

  signals.unshift(signal);
  seenAddresses.add(addressKey);
  if (isVisibleSignal(signal)) announceSignal(signal);
  schedulePersist();
  return true;
}
...
function completeSignal(signal, reason, now = new Date()) {
  ...
  signal.tracking.finalX = isReasonableX(peakX) ? peakX : isReasonableX(currentX) ? currentX : null;
...
function applyPair(signal, pair) {
  ...
  if (!isReasonableX(currentX)) {
    console.warn(`X non valida ignorata per ${signal.ticker || signal.address}: ${currentX}`);
    return false;
  }
  ...
  tracking.marketConfirmedAt ||= now;
      ...
      for (let index = 0; index < group.length; index += 30) {
        ...
        try {
          ...
          for (const signal of chunk) {
            ...
            const wasVisible = isVisibleSignal(signal);
...
async function scrapePublicChannel() {
  ...
  try {
    ...
    if (!scraperInitialized) {
      for (const { post, signal } of classifiedPosts) {
        if (signal) {
          const postTime = post.date ? post.date * 1000 : Date.now();
          const recentEnough = Date.now() - postTime <= recentBackfillWindow;
          if (recentEnough) {
            const published = await publish(signal);
            if (published || isKnownInvalidSignal(signal)) seenPosts.add(post.post);
            continue;
          }
        }
      ...
      for (const { post, signal } of validSignals) {
        if (!seenPosts.has(post.post)) {
          const published = await publish(signal);
          if (published || isKnownInvalidSignal(signal)) seenPosts.add(post.post);
        }
      }
      for (const { post, signal } of classifiedPosts) {
        if (!signal) seenPosts.add(post.post);
      }
      schedulePersist();
    }
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "GET" && url.pathname === "/api/signals") {
      ...
      const visibleSignals = signals.filter(isVisibleSignal);
      return json(res, 200, {
        signals: visibleSignals.map(publicSignal),
    ...
    if (req.method === "GET" && url.pathname === "/api/status") {
      if (!await requireMember(req, res)) return;
      const visibleSignals = signals.filter(isVisibleSignal);
      const activeSignals = visibleSignals.filter((signal) => signal.tracking?.lifecycle !== "completed").length;
      return json(res, 200, {
        ok: !sourceStatus.lastError,
        source: publicSourceStatus(),
        signalCount: visibleSignals.length,
        activeSignals,
        completedSignals: visibleSignals.length - activeSignals,
